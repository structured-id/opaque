...
This is an OPAQUE (RFC 9807) client library for TypeScript/JavaScript. It provides password-authenticated key exchange (PAKE) without the server ever seeing the password. The current implementation uses WebCrypto placeholders; a custom WASM module (Rust/Pallas curve) will replace core crypto primitives later.

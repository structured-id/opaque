...
/**
 * Multi-column permutation grand-product Z for one chunk (halo2 permutation::prover,
 * chunk_len columns per Z). modified[i] = ∏_j (col_j[i] + δ^(globalⱼ)·ωⁱ·β + γ) /
 * (col_j[i] + β·σ_j[i] + γ); Z[0]=lastZ; Z[i]=Z[i-1]·modified[i-1]. deltaPows[j] =
 * δ raised to the global permutation-column index of chunk column j.
 */
export function permutationZChunk(
  cols: bigint[][],
  sigmas: bigint[][],
  deltaPows: bigint[],
  beta: bigint,
  ...
  const n = 1 << k;
  const omega = omegaForSize(k);
  const modified = new Array(n).fill(1n);
  for (let j = 0; j < cols.length; j++) {
    const col = cols[j];
    const sigma = sigmas[j];
    let dw = deltaPows[j];
    for (let i = 0; i < n; i++) {
      const num = Fp.add(Fp.add(Fp.mul(dw, beta), gamma), col[i]);
      const den = Fp.add(Fp.add(col[i], Fp.mul(beta, sigma[i])), gamma);
      modified[i] = Fp.mul(modified[i], Fp.mul(num, Fp.inv(den)));
      dw = Fp.mul(dw, omega);
    }
  }
  const z = [lastZ];

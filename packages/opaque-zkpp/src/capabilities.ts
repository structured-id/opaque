 ...
 *
 * Kernel tiers (fastest → slowest), priority wasm128 → wasm → ts-workers → pure-ts:
 *   wasm-simd-threaded  — SIMD128 + worker-pool threads (needs cross-origin isolation)
 *   wasm-simd           — SIMD128, single-thread        ("wasm128")
...
/** Pick the fastest kernel the platform can run (wasm128 -> wasm -> ts-workers -> pure-ts). */
export function selectKernel(c: Capabilities = detectCapabilities()): Kernel {
  if (!c.wasm) return c.workers ? "ts-threaded" : "ts";

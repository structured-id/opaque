...
function makeCurve(P: bigint, N: bigint, Gx: bigint, Gy: bigint): Curve {
  ...
  const C = weierstrass({ p: P, n: N, h: 1n, a: 0n, b: B, Gx, Gy, Fp: NFp, Fn: NFn });

  const GENERATOR = { x: F.mod(Gx), y: F.mod(Gy) };
  const toNoble = (p: { x: bigint; y: bigint }) => C.fromAffine({ x: p.x, y: p.y });
  ...
  const neg = (p: Point): Point => (p === null ? null : { x: p.x, y: F.neg(p.y) });
  const double = (p: Point): Point => (p === null ? null : fromNoble(toNoble(p).double()));
